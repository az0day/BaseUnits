...
namespace ZZXUnit.Tests.Serializers
{
    ...
    /// </summary>
    public class SnappyStandardPerformance : BaseSerializerPerformanceTest
    {
        private readonly ITestOutputHelper _output;

        public SnappyStandardPerformance(ITestOutputHelper outputHelper)
        {
        ...
        public void SnappyStandard_Test(int rows, int count, long hdp, long ou)
        {
            ...
            {
                st.Restart();
                bytes = pe.ToSnappyStandardBytes();
                ms = st.ElapsedMilliseconds;

                var length = bytes.Length;
                var avg = (olength * 1000M) / (ms * M1);
                _output.WriteLine(@"ToSnappyStandardBytes - length: {0:N0}({4:N2}%), {1:N0}ms, {2:N0}M/S - {3}", length, ms, avg, i, (length * 100M) / olength);
            }
            ...
            {
                st.Restart();
                var oo = bytes.FromSnappyStandardBytes<IList<MatchPriceEntity>>();
                prices = oo[0].Prices;
                ms = st.ElapsedMilliseconds;
                _output.WriteLine(@"FromSnappyStandardBytes: {0:N0}ms - {1}", ms, i);
            }
