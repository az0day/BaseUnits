...
namespace BaseUnits.Core.Helpers
{
    ...
    public sealed class ThreadQueueConsumer<T> : IDisposable
    {
        /// <summary>
        /// 用来存储生产者的队列
        /// </summary>
        ...
        public ThreadQueueConsumer(Action<T> consumerAction)
        {
        ...
        private void Consume()
        {
            ...
            while (_running)
            {
                ...
                if (ret == 0)
                {
                    while (_queue.TryDequeue(out var nextItem))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            OnCallbackFailed?.Invoke(this, new ActionCallbackFailedEventArgs<T>(nextItem, ex));
                        }
                        finally
                        {
                ...
                {
                    LogsHelper.Info("Queue-Consumer", $"break. #{ret}");
                    break;
                }
            }
        }
        ...
        /// 错误信息
        /// </summary>
        public class ActionCallbackFailedEventArgs<TValue> : EventArgs
        {
            public readonly TValue Value;
            public readonly Exception Exception;

            public ActionCallbackFailedEventArgs(TValue value, Exception exception)
            {
                Value = value;
                Exception = exception;
            }
